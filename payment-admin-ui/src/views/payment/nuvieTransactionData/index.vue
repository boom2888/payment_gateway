<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['payment:nuvieTransactionData:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['payment:nuvieTransactionData:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['payment:nuvieTransactionData:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['payment:nuvieTransactionData:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="nuvieTransactionDataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="欧元汇率" align="center" prop="quote" />
      ...
      <el-table-column label="加密货币金额" align="center" prop="cryptoAmount" />
      ...
      <el-table-column label="Nuvei交易ID" align="center" prop="transactionId" />
      <el-table-column label="卡发卡国家" align="center" prop="issuerCountry" />
    ...
    <el-col :span="12">
        <el-form-item label="欧元汇率" prop="quote">
          <el-input v-model="form.quote" placeholder="请输入欧元汇率" />
        </el-form-item>
    </el-col>
    ...
    <el-col :span="12">
        <el-form-item label="加密货币金额" prop="cryptoAmount">
          <el-input v-model="form.cryptoAmount" placeholder="请输入加密货币金额" />
        </el-form-item>
    </el-col>
    ...
    <el-col :span="12">
        <el-form-item label="Nuvei交易ID" prop="transactionId">
          <el-input v-model="form.transactionId" placeholder="请输入Nuvei交易ID" />
        </el-form-item>
    </el-col>


    <el-col :span="12">
        <el-form-item label="卡发卡国家" prop="issuerCountry">
          <el-input v-model="form.issuerCountry" placeholder="请输入卡发卡国家" />
        </el-form-item>
    </el-col>
...
<script>
import { listNuvieTransactionData, getNuvieTransactionData, delNuvieTransactionData, addNuvieTransactionData, updateNuvieTransactionData, exportNuvieTransactionData } from "@/api/payment/nuvieTransactionData";

export default {
  name: "NuvieTransactionData",
  components: {
  },
  data() {
    return {
      ...
      rules: {
        ...
        finalAmount: [
          { required: true, message: "欧元金额不能为空", trigger: "blur" }
        ],
        orderCreated: [
          { required: true, message: "订单表创建时间不能为空", trigger: "blur" }
        ],
        cryptoAmount: [
          { required: true, message: "加密货币金额不能为空", trigger: "blur" }
        ],
        orderStatus: [
        ...
        transactionId: [
          { required: true, message: "Nuvei交易ID不能为空", trigger: "blur" }
        ],
        issuerCountry: [
          { required: true, message: "卡发卡国家不能为空", trigger: "blur" }
        ]
      }
    };
  },
  created() {
    this.getList();
    this.getDicts("NUVIE_TRANSACTION_DATA_METHOD_NAME").then(response => {
      this.methodNameOptions = response.data;
    });
    this.getDicts("NUVIE_TRANSACTION_DATA_ORDER_STATUS").then(response => {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listNuvieTransactionData(this.queryParams).then(response => {
        this.nuvieTransactionDataList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加Nuvei交易数据";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getNuvieTransactionData(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改Nuvei交易数据";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateNuvieTransactionData(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addNuvieTransactionData(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除Nuvei交易数据编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delNuvieTransactionData(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有Nuvei交易数据数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportNuvieTransactionData(queryParams);
        }).then(response => {

  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="类型" prop="deleted">
        <el-select v-model="queryParams.deleted" placeholder="请选择类型" clearable size="small" filterable>
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['payment:houseWallet:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['payment:houseWallet:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['payment:houseWallet:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['payment:houseWallet:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="houseWalletList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="订单ID" align="center" prop="id" v-if="false"/>
      <el-table-column label="加密货币ID" align="center" prop="cryptoId" />
    ...
    <el-col :span="12">
        <el-form-item label="加密货币ID" prop="cryptoId">
          <el-input v-model="form.cryptoId" placeholder="请输入加密货币ID" />
        </el-form-item>
    </el-col>
...
<script>
import { listHouseWallet, getHouseWallet, delHouseWallet, addHouseWallet, updateHouseWallet, exportHouseWallet } from "@/api/payment/houseWallet";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        cryptoId: [
          { required: true, message: "加密货币ID不能为空", trigger: "blur" }
        ],
        createdAt: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHouseWallet(this.queryParams).then(response => {
        this.houseWalletList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getHouseWallet(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateHouseWallet(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addHouseWallet(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除平台钱包编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delHouseWallet(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有平台钱包数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportHouseWallet(queryParams);
        }).then(response => {

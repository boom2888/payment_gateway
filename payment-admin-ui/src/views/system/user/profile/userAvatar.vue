<template>
  <div>
    <div class="user-info-head" @click="editCropper()">
      <img :src="options.img" :title="$t('system.profile.clickToUpload')" class="img-circle img-lg" />
    </div>
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body @opened="modalOpened">
      ...
      <el-row>
        ...
        <el-col :lg="{ span: 2, offset: 6 }" :md="2">
          <el-button type="primary" size="small" @click="uploadImg()">{{ $t('system.profile.submit') }}</el-button>
        </el-col>
      </el-row>
    </el-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      visible: false,
      // 弹出层标题
      title: this.$t('system.profile.uploadAvatar'),
      options: {
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf('image/') == -1) {
        this.msgError(this.$t('system.profile.fileFormatError'))
      } else {

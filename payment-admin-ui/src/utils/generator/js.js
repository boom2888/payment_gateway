  ...
  if (trigger[conf.tag]) {
    ...
    if (conf.regList && isArray(conf.regList)) {
      conf.regList.forEach(item => {
        if (item.pattern) {
          const pattern = toRegExpLiteral(item.pattern)
          if (pattern) {
...
function toRegExpLiteral(value) {
  if (value instanceof RegExp) return value.toString()
  const match = String(value).trim().match(/^\/(.*)\/([dgimsuvy]*)$/)
  if (!match) return null
  try {
    return new RegExp(match[1], match[2]).toString()
  } catch (e) {
    return null
  }
}
